...
class ConcretePlanner():
    ...
    def adapt_plan(self, tools: set[RegisteredTool], abs_tools: list[dict], abs_code: str):
        ...
        for abs_tool in abs_tools:
            selected_tool = self.__match_tool(tools, abs_tool) # Return value/type of __match_tool() currently unknown        
            matches[abs_tool['name']] = selected_tool
                
        if self.debug:
            for abs_name in matches:
                print(f"Matched tool for {abs_name}: {matches[abs_name].get_name()}\n")
                print("\n")
        ...
        if "main" in exec_scope:
            ...
            if self.debug: print(f"RESULTS:\n", result)
        else:
            if self.debug: print(f"Results NOT FOUND")
        if self.debug: print("\n")
    ...
    def __match_tool(self, tools: set[RegisteredTool], abstract_tool: dict) -> RegisteredTool:
        ...
        for tool in tools:
            ...
            doc = Document(page_content=(tool.get_name() + ": " + tool.get_description() +  tool.input_str() + tool.output_str()))
            docs.append(doc)
            
        faiss_store = FAISS.from_documents(docs, embeddings)
        
        compstr: str = abstract_tool["name"] + ": " + abstract_tool["description"]
        compstr += "\nInputs:"
        ...
        retrieved_docs_with_scores = faiss_store.similarity_search_with_score(compstr)
